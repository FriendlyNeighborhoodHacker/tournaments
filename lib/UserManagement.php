...
class UserManagement {
  ...
  private static function log(string $action, ?int $userId, array $details = []): void {
    // no-op for now; will record to an activity log table later
  }
  ...
  public static function createAdmin(array $data): int {
    ...
    $isCoach = self::boolInt($data['is_coach'] ?? 0);
    $isAdmin = self::boolInt($data['is_admin'] ?? 0);

    $st = self::pdo()->prepare(
      "INSERT INTO users (first_name,last_name,email,phone,password_hash,is_coach,is_admin,email_verify_token,email_verified_at)
       VALUES (?,?,?,?,?,?,?,NULL,NOW())"
    );
    $st->execute([$first, $last, $email, $phone, $hash, $isCoach, $isAdmin]);
    $id = (int)self::pdo()->lastInsertId();
    self::log('user.create_admin', $id, ['email' => $email, 'is_coach' => $isCoach, 'is_admin' => $isAdmin]);
    return $id;
  }

  // Self-registration creates a non-admin/non-coach user with a verification token
  public static function createSelf(array $data, string $verifyToken): int {
    ...
    $st->execute([$first, $last, $email, $phone, $hash, $verifyToken]);
    $id = (int)self::pdo()->lastInsertId();
    self::log('user.create_self', $id, ['email' => $email]);
    return $id;
  }

  public static function updateWithPassword(int $id, array $data, string $newPassword): bool {
    $first = self::str($data['first_name'] ?? '');
    $last  = self::str($data['last_name'] ?? '');
    $email = self::normEmail($data['email'] ?? '');
    $phone = self::nullableStr($data['phone'] ?? null);
    ...
    if ($first === '' || $last === '' || !$email || $newPassword === '') {
      throw new InvalidArgumentException('Missing required fields for user update with password.');
    }

    $hash = password_hash($newPassword, PASSWORD_DEFAULT);
    $st = self::pdo()->prepare(
      "UPDATE users SET first_name=?, last_name=?, email=?, phone=?, password_hash=?, is_coach=?, is_admin=? WHERE id=?"
    );
    $ok = $st->execute([$first, $last, $email, $phone, $hash, $isCoach, $isAdmin, $id]);
    if ($ok) self::log('user.update_with_password', $id, ['email' => $email, 'is_coach' => $isCoach, 'is_admin' => $isAdmin]);
    return $ok;
  }

  public static function updateWithoutPassword(int $id, array $data): bool {
    $first = self::str($data['first_name'] ?? '');
    $last  = self::str($data['last_name'] ?? '');
    $email = self::normEmail($data['email'] ?? '');
    $phone = self::nullableStr($data['phone'] ?? null);
    ...
    if ($first === '' || $last === '' || !$email) {
      throw new InvalidArgumentException('Missing required fields for user update.');
    }

    $st = self::pdo()->prepare(
      "UPDATE users SET first_name=?, last_name=?, email=?, phone=?, is_coach=?, is_admin=? WHERE id=?"
    );
    $ok = $st->execute([$first, $last, $email, $phone, $isCoach, $isAdmin, $id]);
    if ($ok) self::log('user.update', $id, ['email' => $email, 'is_coach' => $isCoach, 'is_admin' => $isAdmin]);
    return $ok;
  }
  ...
  public static function setPasswordResetToken(int $id, string $tokenHash, int $minutesValid = 30): bool {
    ...
    // Compute expiration timestamp in server time (DB time zone should be UTC per schema setup)
